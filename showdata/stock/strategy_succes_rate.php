...
while($row=mysql_fetch_row($res)){
	...
	while($result = mysql_fetch_row($res_stock)){
		...
		echo $flagg++;
		echo "++++++++++++++++";
		if ($result[9] >= $price_selected*1.1){
			$waste_time++;
			echo "%%%%%%%%%";
			echo $result[9];
			echo "^^^^^^";
			echo $price_selected*1.1;
			echo "%%%%%%%%%%%";
			break;
		}else{
			$waste_time++;
...
$sql_sucess = "delete from `sucess` where date >= '$date_array[0]' and strategy = '10%_to_sell'";
$conn=mysql_connect("localhost","root","root");
if(!$conn){
	echo "连接失败";
}
...
$sql_sucess = "insert into `sucess` (date, waste, strategy) values ('$date_array[0]', '$average_day', '10%_to_sell')";
echo $sql_sucess;
$conn=mysql_connect("localhost","root","root");
if(!$conn){
	echo "连接失败";
